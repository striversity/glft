...
var countAbove = func(cutOffPrice float64) (count uint) {
...
func main() {
	// TODO 2 - count how many items cost $1000.00 or more in db1
	cutOff := 1000.0
	c := countAbove(cutOff)
	fmt.Printf("There are %v items which cost $%.2f or more\n", c, cutOff)

	// TODO 3 - count how many items cost $ 500.00 or more in db2
	cutOff = 500
	c = countAbove(cutOff)
	fmt.Printf("There are %v items which cost $%.2f or more\n", c, cutOff)
}
