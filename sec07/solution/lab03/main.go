...
var (
	...
	filterWords = map[string]bool{
		"is": true, "to": true, "be": true, "in": true, "of": true,
		"for": true, "or": true, "not": true, "an": true, "are": true,
		"by": true,
	}
)
...
func main() {
	...
	for _, fn := range inputFiles {
		...
		out = filterProcessor(out) // processor
		countWords(out)            // sink
	}
...
func filterProcessor(in <-chan string) (out <-chan string) {
	wg.Add(1)
	c := make(chan string)
